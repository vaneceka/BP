...
class MissingOrWrongFormulaOrNotCalculatedCheck(BaseCheck):
    name = "Zcela chybí výpočet / vzorec je chybný nebo není vypočten"
    penalty = -10
    ...
    def run(self, document, assignment=None) -> CheckResult:
        ...
        for addr, spec in assignment.cells.items():
            ...
            if not actual or not actual.startswith("="):
                errors.append(f"{sheet}!{addr}: chybí vzorec")
                continue

            act = document.normalize_formula(actual)
            expected = document.normalize_formula(expected)

            if act != expected:
                errors.append(
                    f"{sheet}!{addr}: špatný vzorec "
                    f"(oček. {expected}, nalezeno {actual})"
                )

            if info["is_error"]:
                errors.append(f"{sheet}!{addr}: výsledek je chyba")
                continue
